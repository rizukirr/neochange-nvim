...
local function create_floating_window(content, title)
    ...
    if title then
        ...
        for i, line in ipairs(content) do
            vim.api.nvim_buf_set_lines(buf, title_start_line + i - 1, title_start_line + i - 1, false, {line})
        end
    else
...
function M.show_branch_selector()
    ...
    if err then
        vim.notify("Failed to get current branch: " .. err, vim.log.levels.ERROR)
        return
    end
    ...
    local buf, win = create_floating_window(display_lines, "NeoChange - Select Configuration Branch")
